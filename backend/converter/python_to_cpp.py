...
class PythonToCppConverter(BaseConverter):
    ...
    def _generate_function(self, node: Dict, level: int) -> str:
        ...
        name = node.get('name')
        return_type = self._map_cpp_type(node.get('returnType', 'void'))
        params = node.get('params', [])
        
        # Generate parameter list
        param_strs = []
        for param in params:
            param_type = self._map_cpp_type(param.get('inferredType', 'int'))
    ...
    def _generate_assign(self, node: Dict, level: int) -> str:
        ...
        inferred_type = self._map_cpp_type(node.get('inferredType', 'int'))
    ...
    def _generate_print(self, node: Dict, level: int) -> str:
        ...
        if not args:
            return f'{self._indent(level)}cout << endl;'
        ...
        cout_chain = ' << " " << '.join(parts)
        return f'{self._indent(level)}cout << {cout_chain} << endl;'

    def _generate_tuple_swap(self, node: Dict, level: int) -> str:
        """Generate tuple swap: auto _tmp = a; a = b; b = _tmp;"""
        targets = node.get('targets', [])
        values = node.get('values', [])
        if len(targets) == 2 and len(values) == 2:
            a, b = targets[0], targets[1]
            lines = [
                f'{self._indent(level)}auto _tmp_{a} = {a};',
    ...
    def _generate_input(self, node: Dict, level: int) -> str:
        ...
        cpp_type = self._map_cpp_type(inferred_type)
        
        # Print prompt
        if prompt:
            prompt_str = self._generate_expr(prompt)
            lines.append(f'{self._indent(level)}cout << {prompt_str};')
        
        # Declare variable
        lines.append(f'{self._indent(level)}{cpp_type} {target};')
        
        # Read input
        lines.append(f'{self._indent(level)}cin >> {target};')
    ...
    def _map_cpp_type(self, py_type: str) -> str:
